...
**绝对保真于炉石传说是硬性要求**：卡牌效果、结算/触发顺序、目标规则与资源机制必须与真实炉石语义一致，以 RosettaStone/SabberStone 为正确性基准（执行机制见 Phase 3 的对局结果对比验证与 `docs/architecture-roadmap.md` 里程碑 F）。以下 RL 诉求是同一引擎的工程属性，而非简化游戏规则的许可；任何已记录的简化（过载法力锁定、永久潜行、抉择自动随机等）都是待消除的欠债，不得新增。
