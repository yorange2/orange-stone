...
fn evaluate_effect_value(effect: crate::core::effect::CardEffect) -> f64 {
    ...
    match effect {
        ...
        // M3-W2b — the Across the Timeways legendary wave (fidelity-debt §21).
        CardEffect::SummonRandomMinionFromDeck => 5.0,
        CardEffect::SylvanasDealToAllEnemiesRepeated { damage } => damage as f64 * 1.6,
        CardEffect::ChronogorDrawsHighestLowest => 5.0,
        CardEffect::DiscardHandAndAddInfiniteBanana => 3.0,
        CardEffect::MurozondPrepareInfiniteAttack => 5.0,
        CardEffect::AddCopiesOfLastPlayedCards { count } => count as f64 * 1.2,
        CardEffect::SummonLocationForPlayer { .. } => 3.0,
        CardEffect::SummonCopyOfFriendlyMinion => 4.0,
        CardEffect::FillHandWithRandomTemporarySpells => 4.0,
        CardEffect::TakeControlEnemyMinionHealthLE => 6.0,
        CardEffect::DiscoverDemonGE5AndSetNextDemonCostOne => 4.0,
        CardEffect::SpendCorpsesRestoreHeroHealth { max } => max as f64 * 0.4,
        CardEffect::DrawOrResurrectBwonsamdiAndGrantBoon { .. } => 4.0,
        CardEffect::SummonRandomCostMinion { cost } => 3.0 + cost as f64 * 0.3,
        CardEffect::SetDeckBottomCostsOne { count } => count as f64 * 0.5,
        CardEffect::ReplaceHandAndSwapBackAtTurnEnd => 3.0,
        CardEffect::RestoreHandSnapshot => 1.0,
        CardEffect::SummonChestForOpponent => 1.0,
        CardEffect::FillOpponentHandWithCoins => -1.0,
        CardEffect::DestroyAllMinionsOpponentPlayedLastTurn => 6.0,
        CardEffect::SummonBloodFighterFromHandBuffAndAttack => 6.0,
        CardEffect::GetThreeRandomSpellsFromPastTracked => 4.0,
        CardEffect::DestroyHeldKingLlaneAndHalveEnemyHealth => 6.0,
        CardEffect::SilenceAndDestroyAllOtherMinions => 8.0,
        // M3-W3 — The End of Time miniset (fidelity-debt §22).
        CardEffect::DealDamageAndImbue { amount, .. } => amount as f64 * 1.4,
        CardEffect::DrawUndeadAndImbueTwice => 4.0,
        CardEffect::EquipDaggerOrBuffWeapon => 3.0,
        CardEffect::BygoneEchoesSummon => 5.0,
        CardEffect::ChronikarHeroAttackBuff => 3.0,
        CardEffect::ChronikarRebuff => 1.0,
        CardEffect::PressTheAdvantage => 3.0,
        CardEffect::RefreshManaCrystals { amount } => amount as f64 * 1.5,
        CardEffect::SummonTwoTreantsScaling => 4.0,
        CardEffect::SetAllOtherMinionsAttack { .. } => 3.0,
        CardEffect::SetAllOtherMinionsHealth { .. } => 3.0,
        CardEffect::ArmAccelerationAura => 3.0,
        CardEffect::SetWeaponAttackInfinityThisTurn => 4.0,
        CardEffect::DamageAndBuffFriendlyIfKilled { amount, .. } => amount as f64 * 1.3,
        CardEffect::AddRandomDeathrattleMinionCostsLess => 4.0,
        CardEffect::DiscardHighestCostCard => -2.0,
        CardEffect::DrawUntilHandFull => 4.0,
        CardEffect::EmptyOpponentHand => 6.0,
        CardEffect::SetRandomHandCardCostInfinity => 3.0,
        CardEffect::RestoreInfinityHandCardCost => 1.0,
        CardEffect::GainStatsIfHeroDamagedThisTurn { .. } => 2.0,
        CardEffect::DamageMinionDrawIfSurvivesSummonIfDies { .. } => 2.5,
        CardEffect::BuffHandMinionsAndWeapons { attack } => attack as f64 * 2.0,
        CardEffect::FreezeMinionAndNeighborsDestroyDamaged => 4.0,
        CardEffect::InfiniteDamageToHighestHealthEnemyMinion => 8.0,
        CardEffect::DamageMinionEternalFirebolt { amount } => amount as f64 * 1.2,
        CardEffect::DestroyAllMinionsWith4OrLessAttack => 7.0,
        CardEffect::AddRandomShadowSpell => 2.0,
        CardEffect::OverloadForAndGainImmuneWindfury { .. } => 3.0,
        CardEffect::DestroyRandomEnemyMinionLocationWeapon => 6.0,
        CardEffect::DestroyTopFiveEnemyDeckIfOwnEmpty => 5.0,
        ...
        // 2025–2026 expansions M4-W4 (Deathwing + remaining Cataclysm cards — §26).
        CardEffect::ReplaceHero { .. } => 10.0,
        CardEffect::ChooseCataclysms => 8.0,
        CardEffect::ChooseHandCard => 4.0,
        CardEffect::RefreshManaIfHoldingDragon { amount } => amount as f64 * 1.5,
        CardEffect::GainTempManaOrPermanentIfSpent { .. } => 3.0,
        CardEffect::GetOrSummonTauntWhelpsIfSpent { .. } => 3.0,
        CardEffect::SummonGolemsSpendAllMana => 5.0,
        CardEffect::ShuffleRandomMinionsCostGE8DoubleStats => 6.0,
        CardEffect::GainStatsPerFriendlyMinionTargeted => 2.0,
        CardEffect::FillHandWithRandomDragons { .. } => 4.0,
        CardEffect::SetAttackEqualToSource => 2.0,
        CardEffect::NextMurlocCostsHealth => 2.0,
        CardEffect::TransformRandomEnemyMinionToSelf => 6.0,
        CardEffect::GiveOpponentSabotage => -1.0,
        CardEffect::ReturnEnemyMinionCantPlayNextTurn => 5.0,
        CardEffect::SetHealingBonus { amount } => amount as f64 * 1.0,
        CardEffect::SetNextHealDealsDamage => 2.0,
        CardEffect::FullHealMinionAndDraw => 4.0,
        CardEffect::DamageMinionHealEnemyHeroIfKilled { amount } => amount as f64 * 1.0,
        CardEffect::LifestealSelfDamage { amount } => amount as f64 * 1.2,
        CardEffect::GainHealthIfFullHealth { amount } => amount as f64 * 0.8,
        CardEffect::SetRemainingHealthAndFullHealDamage { health, damage } => {
            health as f64 * 0.8 + damage as f64 * 1.0
        }
        CardEffect::BuffSpellDamageHandAndDeck => 3.0,
        CardEffect::AddDragonBreathScaledByAttack => 3.0,
        CardEffect::DealDamageEqualDragonBreath => 3.0,
        CardEffect::SummonFiveHungryDrakesSpendCorpsesRush => 6.0,
        CardEffect::RefreshManaEqualSelfAttack => 3.0,
        CardEffect::AddNefariansCreation { reduction } => 4.0 + reduction as f64 * 0.5,
        CardEffect::GrantDeathrattleSummonRandomCostMinion { cost } => 3.0 + cost as f64 * 0.3,
        CardEffect::TriggerRandomFriendlyEndTurnEffect => 3.0,
        CardEffect::GrantDivineShieldOrGainStats { attack, health } => {
            (attack + health) as f64 * 0.8 + 2.0
        }
        CardEffect::AddRandomHolySpellCostReduced { .. } => 3.0,
        CardEffect::SummonDragonWithSelfStats => 4.0,
        CardEffect::SetEndTurnEffectsTwice { .. } => 3.0,
        CardEffect::DevourTwoEnemyHandCardsAndDormant => 5.0,
        CardEffect::ReturnDevouredCards => 3.0,
        CardEffect::SummonCopyIfSpellDamageDealtThisTurn => 3.0,
        CardEffect::DealDamageAndDamageRandomEnemyMinion { amount, secondary } => {
            amount as f64 * 1.2 + secondary as f64 * 0.6
        }
        CardEffect::GainAttackFirstSpellDamageThisTurn { attack } => attack as f64 * 0.8,
        CardEffect::DamageAllMinionsRepeatDescending { amount } => amount as f64 * 1.0,
        CardEffect::SummonCopyOfDiscardedMinion => 4.0,
        CardEffect::TakeControlUntilEndOfTurnCantAttack => 5.0,
        CardEffect::DealDamageToTwoScaledByHandTurns { base } => base as f64 * 1.5,
        CardEffect::DamageAllMinionsDrawPerDeath { amount } => amount as f64 * 1.2 + 1.0,
        CardEffect::ReopenLocation => 4.0,
        CardEffect::SetNextTurnSummon { .. } => 3.0,
        CardEffect::DealDamageLeftRightOutcastAgain { amount } => amount as f64 * 1.4,
        CardEffect::DealDamageEqualSelfAttack => 3.0,
        CardEffect::SetDragonsHaveRush => 3.0,
        CardEffect::SetEnemyMinionHealthTo1IfHoldingDragon => 5.0,
        CardEffect::AddRandomDragonCostLE3 => 2.0,
        CardEffect::DealDamageOrDamageAllEnemiesIfCopyPlayed { amount } => amount as f64 * 1.2,
        CardEffect::ReplayOneCostCardsPlayedThisGame => 4.0,
        CardEffect::CastAbsorbedSpell => 4.0,
        CardEffect::GrantMegaWindfuryCantAttackHeroes => 5.0,
        CardEffect::TransformFriendlyMinionsCost1MoreSummonOriginals => 3.0,
        CardEffect::SummonRandomThreeTwoOneCostMinions => 4.0,
        CardEffect::DrawAndReduceCostRepeated { .. } => 3.0,
        CardEffect::DamageAllMinionsScaledByBoard { base } => base as f64 * 1.2,
        CardEffect::DamageAllMinionsAndGainHeroAttack { amount, attack } => {
            amount as f64 * 1.0 + attack as f64
        }
        CardEffect::DealDamageSplitAmongEnemiesIfFireSpell { amount } => amount as f64 * 1.0,
        CardEffect::DamageDamagedMinionReturnIfExcess { amount } => amount as f64 * 1.0,
        CardEffect::SetGeddonDiscoverDraw => 3.0,
        CardEffect::GrantDeathrattleSummonRandomHandMinion => 3.0,
        CardEffect::SummonRandomMinionFromHand => 4.0,
        CardEffect::UpgradeHeroPowerCost1 => 2.0,
        CardEffect::AddRandomPaladinAura => 3.0,
        CardEffect::StealHealthThreeTimes { amount } => amount as f64 * 0.9,
        CardEffect::DestroyDeckCardsCostLE2Both => 5.0,
        CardEffect::UnlockOverloadedCrystals => 2.0,
        CardEffect::CastRandomSpellSameCostOtherClass => 3.0,
        CardEffect::ReturnHoardCostLess => 4.0,
        CardEffect::DamageMinionReduceHandCostByExcess { amount } => amount as f64 * 1.0,
        CardEffect::AddTwoRandomSpellsSameCost { cost } => 2.0 + cost as f64 * 0.3,
        CardEffect::Split100StatsAmongDeckMinionsIfCostGE100 { .. } => 6.0,
        CardEffect::DestroyHighestHealthEnemyMinion => 4.0,
        CardEffect::ShuffleRandomLegendaryDragonsCost1 => 4.0,
        CardEffect::DestroyLegendaryMinion => 4.0,
        CardEffect::GrantRandomFriendlyMinionAttack { attack } => attack as f64 * 1.5,
        CardEffect::DiscoverOneCostSpell => 2.5,
        CardEffect::DiscoverAnySpell => 2.5,
        CardEffect::SummonTwoRandomOneCostMinions => 3.0,
        CardEffect::ReopenLocationIfFelSpell => 3.0,
        CardEffect::SummonRandomMinionsOfCost { count, .. } => count as f64 * 3.0,
        CardEffect::AddRandomNagaCost1 => 2.0,
        CardEffect::HoggerStartOfGame => 4.0,
        CardEffect::AzalinaStartOfGame => 5.0,
        CardEffect::GodfreyStartOfGame => 2.0,
        CardEffect::NethrekStartOfGame => 3.0,
        CardEffect::MugzeeStartOfGame => 3.0,
        CardEffect::BeatrixStartOfGame => 3.0,
        CardEffect::AyaUpgradeCoins { .. } => 3.0,
        CardEffect::KarovThreeLegendaryCopies => 4.0,
        CardEffect::ThalenaSecondHeroPower => 3.0,
        CardEffect::ManastormSetAfterSpell => 3.0,
        CardEffect::VanessaGetBattlecryMinionCost2Less => 3.0,
        CardEffect::TrastathGainSummonedDemonStats => 3.0,
        CardEffect::MoraggDeathrattle => 4.0,
        CardEffect::SummonMoragg => 4.0,
        CardEffect::PassiveHeroPower => 0.0,
        CardEffect::JadeCoin => 2.0,
        CardEffect::GrimyCoin => 2.0,
        CardEffect::KabalCoin => 2.0,
        // M5-W2 — the closing Escape from Violet Hold wave (exp_jail_w2.rs)
        CardEffect::SliceAndDice => 6.0,
        CardEffect::TinyPal { .. } => 3.0,
        CardEffect::StaffOfTrickery => 4.0,
        CardEffect::R4TCatcher => 4.0,
        CardEffect::IridaSinseeker => 4.0,
        CardEffect::IridaGetVoid => 3.0,
        CardEffect::KingOfTheUnderbelly => 4.0,
        CardEffect::MurlocHolmes => 4.0,
        CardEffect::TogwaggleShuffleHands => 4.0,
        CardEffect::MaievBuffDormant => 4.0,
        CardEffect::ZuramatPlaysDiscarded => 5.0,
        CardEffect::ColdSnap => 2.0,
        CardEffect::MindSweeper => 3.0,
        CardEffect::EnthralledShade => 2.0,
        CardEffect::TricksyImproviser => 3.0,
        CardEffect::Judgment => 4.0,
        CardEffect::ReinforcementAura => 4.0,
        CardEffect::ScarletBruiser => 2.0,
        CardEffect::BallAndChain => 2.0,
        CardEffect::HolyBola => 2.0,
        CardEffect::SpireSecurity => 3.0,
        CardEffect::SmuggledShovel => 2.0,
        CardEffect::ScrambleForGear => 2.0,
        CardEffect::ReleaseTheBeasts => 2.0,
        CardEffect::SewerSwimmer => 3.0,
        CardEffect::Imfernal => 3.0,
        CardEffect::ImpGangStooge => 3.0,
        CardEffect::DisguisedDoctor => 3.0,
        CardEffect::Sawbones => 4.0,
        CardEffect::BoneFlurry => 3.0,
        CardEffect::DrinkBlood => 3.0,
        CardEffect::EmergencySurgery => 4.0,
        CardEffect::DisguisedWatchman => 2.0,
        CardEffect::PickPocket => 2.0,
        CardEffect::JadeGuardians => 3.0,
        CardEffect::CrowdControl => 3.0,
        CardEffect::VigilantSentry => 3.0,
        CardEffect::VioletPunisher => 3.0,
        CardEffect::BreakoutArchitect => 4.0,
        CardEffect::NoxiousBribe => 3.0,
        CardEffect::AlarmOMatic => 3.0,
        CardEffect::SpitefulChef => 3.0,
        CardEffect::Annihilation => 5.0,
        CardEffect::SpireOfSolitude => 4.0,
        CardEffect::ShadowRounds => 3.0,
        CardEffect::ScarletRecruiter => 4.0,
        CardEffect::ThievesTools => 3.0,
        CardEffect::VoidSoul => 2.0,
        CardEffect::CodeViolet => 4.0,
        CardEffect::MoltenGold => 2.0,
        CardEffect::Frostshatter => 3.0,
        CardEffect::Stormfury => 4.0,
        CardEffect::Hexmarshal => 3.0,
        CardEffect::LethalRecipe => 3.0,
        CardEffect::DigForFreedom => 3.0,
        CardEffect::GuardDog => 2.0,
        CardEffect::BeastTripwire => 3.0,
        CardEffect::ArcaneTripwire => 3.0,
        CardEffect::CapturedArchmage => 2.0,
        CardEffect::FranticForger => 2.0,
        CardEffect::LowSecurityWing => 2.0,
        CardEffect::DemonicConfinement => 2.0,
        CardEffect::WidowsBite => 2.0,
        CardEffect::WidowsFeast => 2.0,
        CardEffect::WidowsBanquet => 3.0,
        CardEffect::Nab => 3.0,
        CardEffect::VoidBlast => 2.0,
        CardEffect::CosmicManifestations => 3.0,
        CardEffect::DefiasWannabe => 3.0,
        CardEffect::Soothsayer => 4.0,
        CardEffect::HolyEmbrace => 2.0,
        CardEffect::RatBurglar => 3.0,
        CardEffect::DarkBribe => 3.0,
        CardEffect::AncientAugurPick => 3.0,
        CardEffect::AncientAugurDeathrattle => 3.0,
        CardEffect::ArachnathidVenom => 3.0,
        CardEffect::TruthSeeker => 3.0,
        CardEffect::ConcealingConfection => 2.0,
        CardEffect::DisguisedExecutioner => 3.0,
        CardEffect::GetawayHogdriver => 3.0,
        CardEffect::EscapeArtist => 3.0,
        CardEffect::BloodClone => 3.0,
        // M5-W2 — the closing Escape from Violet Hold wave (exp_jail_w2.rs)
        CardEffect::GallagioGoon => 2.0,
        CardEffect::BlackMarketOverseer => 2.0,
