...
状态：里程碑 A–G 全部完成（PR #35–#62），里程碑 F 完成（PR #61、#63、#75：F1 过载锁定、F2 潜行、F4 保真审计、F5 差分验证含外部 SabberStone 对照）。剩余工作：F4 的持续逐效果审计——其积压清单即保真债账本（[docs/fidelity-debt-zh.md](fidelity-debt-zh.md)：68 处简化卡标记，2026-08-06 核对）。
...
- [ ] **F4 持续** — 逐卡保真积压：`src/cards/` 里 **67 处简化卡标记**（权威清单见 [docs/fidelity-debt-zh.md](fidelity-debt-zh.md)，英文版 `fidelity-debt.md`）。一张卡只有在"实现 + F5 验证"后才离开账本。2026-08-06 核对的结构性发现（过期注释、Worgen 一行、3 处卡 ID 冲突、10 张不在 `ALL_CARDS`、7 个重复条目、简化债提取器错配）**已解决（PR #77）**；剩下的是逐机制工作：Enrage、种族字段、缺失触发类、条件谓词与账本机制盘点里列的效果原语。**执行计划：[docs/fidelity-debt-roadmap-zh.md](fidelity-debt-roadmap-zh.md)**（英文版 `fidelity-debt-roadmap.md`）——按依赖排序的 8 个 wave（W0 接线 … W7 收尾）。RL 训练卡池（`hearthstone_os/decks.py`）排除本集合，卡池成员的变动都走账本。
...
- [ ] **F5 持续** — 保真债账本的差分覆盖：每张离开 [docs/fidelity-debt-zh.md](fidelity-debt-zh.md) 的卡必须带一个 `tests/differential.rs` 场景（精确目标集合 / 触发时机 / 死亡阶段交互），语义可镜像的另按 `docs/differential_sabberstone.md` 做 SabberStone 牌组级一致性对照——让 F4 积压可机械验证。
