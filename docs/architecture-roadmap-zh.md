...
状态：里程碑 A–D（热路径、伤害管线、RL 准备、规模）已完成（PR #35–#46）。剩余工作：里程碑 E（工程欠债）、里程碑 G（保真引擎架构）与里程碑 F（卡牌级保真 — 依赖 G）。
...
- ❌ **未解决（G1）** — 无游戏步骤状态机。`TurnStarted` 处理器内直接回满法力并抽牌（`src/engine/rules.rs:430-485`），回合开始类奥秘/触发在抽牌*之后*才触发；先手玩家第 1 回合也抽牌（`rules.rs:484`）；`CardDef::start_turn_effect` 已声明但从未接入引擎；`TurnEnded` 先清除临时增益、后触发回合结束效果（`rules.rs:486-551`）。炉石的顺序（回合开始触发 → 法力 → 抽牌；回合结束触发 → 收尾清理）需要 RS/SB 的 GameStep 模型。
- ❌ **未解决（G2）** — 触发检测为即席组件扫描，顺序为稀疏集下标（生成顺序）而非上场顺序；无先手玩家优先规则，无"每当"与"之后"的时序分类。缺失的触发类无法表达：苦痛侍僧被错误建模为亡语（`src/cards/classic_neutral.rs:618`），暴乱狂战士/铸甲师为白板 — 它们的"每当……受到伤害"效果没有机制承载。
...
- [ ] **G1** — 游戏步骤状态机：仿照 RS/SB 的 `GameStep` 状态机建模炉石的结算步骤（回合开始触发 → 法力 → 抽牌 → 行动 → 回合结束触发 → 死亡阶段 → 收尾）；优先级事件队列退化为步骤*内部*的事件流，步骤由状态进入（有待处理死亡 → 进入死亡步骤）。修复回合开始/结束顺序、临时效果过期、首回合抽牌。
- [ ] **G2** — 注册式触发器：以逐实体触发器注册（RS `ITrigger` / SB `TriggerManager`）取代即席 `iter_*_trigger` 扫描：按上场顺序触发、先手玩家优先、显式"每当"与"之后"时序、按区域校验有效性。接入 `start_turn_effect`（已声明但从未触发）；补充缺失的触发类（受伤触发 — 解锁苦痛侍僧、暴乱狂战士、铸甲师）。
