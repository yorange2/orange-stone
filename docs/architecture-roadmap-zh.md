...
状态：里程碑 A–G 全部完成（PR #35–#62），里程碑 F 完成（PR #61、#63：F1 过载锁定、F2 潜行、F4 保真审计、F5 差分验证）。剩余工作：F5 差分的外部 SabberStone 对照运行（需 .NET 环境）与 F4 的持续逐效果审计——其积压清单即保真债账本（[docs/fidelity-debt-zh.md](fidelity-debt-zh.md)：68 处简化卡标记，2026-08-06 核对）。
...
- [ ] **F4 持续** — 逐卡保真积压：`src/cards/` 里 **68 处简化卡标记**（权威清单见 [docs/fidelity-debt-zh.md](fidelity-debt-zh.md)，英文版 `fidelity-debt.md`）。一张卡只有在"实现 + F5 验证"后才离开账本。2026-08-06 核对发现：4 处过期注释（卡已忠实）、Worgen Infiltrator 一行可修、3 处卡 ID 冲突（Mass Dispel 因 `PRIEST_018` 被抢占而不可达）、6 张不在 `ALL_CARDS`、`ALL_CARDS` 有 7 个重复条目。RL 训练卡池（`hearthstone_os/decks.py`）排除本集合，卡池成员的变动都走账本。
- [x] **F5** — 差分验证：以 SabberStone（及/或 RosettaStone）为参考实现，对局结果 / 事件序列的符合性测试，使保真回归可被机械捕获。*(依赖 G1–G9：差分对比的是步骤级结算)* *(PR #63：`tests/differential.rs` 参考场景 + 转录协议文档)*
