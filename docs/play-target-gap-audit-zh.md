...
71 张里放宽了 **67 张**（`AnyEnemy`→`AnyCharacter` 24、`AnyEnemyMinion`/`FriendlyMinion`→`AnyMinion` 43）。
守卫 `tests/declared_targets_resolve.rs` 当场抓出 11 张改完变哑火的卡，**7 个结算 helper**
补了 `AnyMinion` 分支才通过（`GainStatsThisTurn`、`SetAttack`、`DoubleAttack`、`DoubleHealth`、
`SetAttackToHealth`、`GrantWindfury`、`DealHeroAttackDamage`）—— 这正是审计里预言的第 1 号拦路问题，
现在有护栏，改错会当场失败而不是静默。

顺带修的：**恶魔之眼**（CORE_BT_801）的结算写死 `None`，玩家指定的目标被丢掉（T2 漏网的一张）。

**四张抉择卡先不放宽**（Wrath ×2、自然印记、星落）：出牌时选的目标**活不过抉择**——
`resolve_effect` 在抉择分支结算时传的是 `None`。这是与病变虫群同一个两段式缺口，留给下一波。

**双重施法改为沿用原目标**：泰兰德/辛内斯特拉的"施放两次"和 Niri 的重施原先都用"无目标"重结算
（§14.4 的简化）。域没放宽时这看不出问题（随机池里只有敌方）；放宽后复制的那一发可能打到自己脸上。
炉石的规则是两次打同一个目标，已按此修正——Niri 走的是 CardPlayed 触发、拿不到目标，
于是把目标记在 `Player::last_spell_target` 上。

**推翻了 W14 的一个决策**：铁炉堡步枪兵（`NEUTRAL_B07`）"战吼：造成 1 点伤害"，W14（PR #105）
按"敌方范围修正"把它收窄成只能打敌方随从并写进了断言。官方这句裸伤害的措辞与火球术同类，
可以打任意角色（含英雄），本波按官方语义改回 `AnyCharacter`，测试断言同步反转并注明了推翻原因。

**影响**：合法动作变多——样例棋盘（双方各 3 随从 + 火球术 + 变形术）从 **20 条涨到 27 条**（+35%）。
批量吞吐无明显变化（交替测量区间重叠）。**RL 基准需要重跑**：动作空间和随机兜底分布都变了。
