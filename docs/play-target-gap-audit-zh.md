...
- **F-A13（修 T1 时发现的独立 bug）**：引擎的"消灭随从"是用致死伤害实现的，
  于是**圣盾能吃掉一次消灭**（刺杀打白银之手新兵 → 盾破、随从活着）。与目标
  枚举无关，已登记进账本待单独修；`tests/play_targeting.rs` 的棋盘刻意不放
  圣盾随从，避免这个 bug 盖住"是否命中被选中目标"的信号。
