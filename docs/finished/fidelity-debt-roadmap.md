...
> Archived 2026-08-06 — all waves W8–W12 complete (PRs #97–#101); the
> ledger §11 of `docs/fidelity-debt.md` is EMPTY and the RL pool is the full
> 391-card classic pool. Cleared the remaining fidelity-debt
...
### W12 — Remaining mechanics (5 cards): Water Elemental, Cabal Shadow Priest, Prophet Velen, Shiv, Argent Protector ✅ (PR #101)

- Water Elemental: freeze characters damaged **by** this minion — **D2
  decided**: a damage-pipeline check in `Event::DamageDealt` (applied
  before the divine-shield absorption — the shield pops but the target
  still freezes, matching HS)
- Cabal Shadow Priest: battlecry `TakeControl` of an enemy minion with
  attack ≤ 2 (new `TakeControlAttackLE` effect, permanent control)
- Prophet Velen: healing doubled — **D3 decided**: a pipeline hook in
  `resolve_restore_health` (doubles while Velen is on the board; the
  spell-damage half stays the documented +1 rebalance)
- Shiv: 1 damage + draw 1 (`DealDamageAndDraw` — wiring)
- Argent Protector: Battlecry Divine Shield (new `GrantDivineShield`
  effect) — the roadmap's wave accounting swapped this registered card for
  the unregistered Defender of Argus; clearing it empties the ledger

**Acceptance**: ✅ 5 `w12_*` scenarios; `cargo test` green (442);
**RL pool 386 → 391 (ledger §11 empty)**; ✅ this roadmap pair archived to
`docs/finished/`.
...
| W12 remaining mechanics ✅ (PR #101) | 5 | 386 → **391 (ledger empty)** |
