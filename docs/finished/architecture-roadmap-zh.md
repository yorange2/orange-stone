...
状态：里程碑 A–G 全部完成（PR #35–#62），里程碑 F 完成（PR #61、#63、#75：F1 过载锁定、F2 潜行、F4 保真审计、F5 差分验证含外部 SabberStone 对照）。F4 逐效果保真审计的 67 张积压卡已全部清偿（PR #79–#86，W0 接线 … W7 收尾），账本现位于 `docs/fidelity-debt.md`（未来新增简化卡在此登记），执行计划归档至 docs/finished/；F4/F5 持续审计机制保留。
...
- ✅ **已解决（F1、F2，PR #61）** — 过载原只是触发标记（`src/core/component.rs`）；潜行原为永久潜行且目标侧未实现。现 `Overload(pub i32)` 携带每卡过载值（闪电箭 1、野性狼魂 2、叉状闪电 2、闪电风暴 2、图腾魔像 1、熔岩爆裂 2、土元素 3），出牌时累计到 `Player::overload_locked`，下一回合在 ManaRefill 步骤扣除后清零（G5 预留的生效点）；潜行目标侧随 G9 落实（隐身不可被单目标指定，指定即空发），攻击侧随 F2 落实（攻击后移除潜行）。
...
- ✅ **已解决（E4，PR #62）** — 卡牌数据库原为约 400 个手写 `const CardDef`，`build.rs` 管线只带 4 张样例卡。现完整官方数据库入库（`cards/cards.json`：HearthstoneJSON 经典时代构筑卡牌 — EXPERT1 + CORE + LEGACY 按 ID 去重，共 1190 张）：`build.rs` 批量生成全部静态卡牌常量 + `find_by_id`/`find_by_name` 查找（官方格式武器耐久存于 health 字段 — 已映射；名称索引优先实体卡而非附魔 token）。手写池（自定义 ID）与官方库按**名称**逐字段比对验证，附 90 余条官方重平衡（nerf/buff）差异账本（`known_rebalanced`）。手写效果字段叠加到官方 ID 上（名称→ID 映射）为后续工作。
...
- [x] **E2** — 退役最后一个队列改写原语 `redirect_damages`（B2 遗留）：让法术扭曲者接入伤害管线（例如在 `DamageDealt` 处做伤害来源拦截），而非改写待结算的法术伤害事件。*(⏩ 并入 G8：步骤边界拦截取代队列改写——经 G8 完成，PR #59)*
- [x] **E3** — 抉择/连击决策面（P2）：在 `Action` 中暴露抉择，让 agent 决策而非引擎随机 / 自动判定连击。*(⏩ 由 G6 取代：抉择系统覆盖——经 G6 完成，PR #57)*
- [x] **E4** — 完成 D2：将完整官方 CardDefs JSON 入库并重新生成静态卡牌常量（手写效果字段叠加在生成的静态属性之上）。*(PR #62：`cards/cards.json` 1190 张 + 全量生成 + 名称比对验证与重平衡账本；效果叠加映射为后续)*
...
- [x] **F1** — 过载法力锁定：按真实炉石语义实现（每张牌各自的过载数值在下一回合锁定拥有者的法力），取代仅作触发标记的现状。*(由 G1/G5 解锁：锁定在法力回满步骤生效)* *(PR #61：`Overload(i32)` + `overload_locked`，ManaRefill 扣除)*
...
- [x] **F4 持续** — 逐卡保真积压：`src/cards/` 里 **67 处简化卡标记**（权威清单见 [docs/fidelity-debt-zh.md](../fidelity-debt-zh.md)，英文版 `fidelity-debt.md`）。一张卡只有在"实现 + F5 验证"后才离开账本。2026-08-06 核对的结构性发现（过期注释、Worgen 一行、3 处卡 ID 冲突、10 张不在 `ALL_CARDS`、7 个重复条目、简化债提取器错配）**已解决（PR #77）**。**全部清偿（2026-08-06，PR #79–#86）**：8 个 wave（W0 接线 … W7 收尾，[docs/finished/fidelity-debt-roadmap-zh.md](fidelity-debt-roadmap-zh.md)）全部落地，账本为空，RL 卡池到全经典构筑规模（391）；每张卡离账本时都带 F5 差分场景。事后修正：顺手牵羊的 `OtherClass` 卡池从"任意非潜行者"收窄为另外 8 个职业的职业卡（中立不算职业卡，`is_other_class_card` 钉住）。
...
- [x] **F5 持续** — 保真债账本的差分覆盖：**随账本一起完成（PR #79–#86）**——每张离开 [docs/fidelity-debt-zh.md](../fidelity-debt-zh.md) 的卡都带一个 `tests/differential.rs` 场景（`w0_*`–`w7_*` 共 72 个：精确目标集合 / 触发时机 / 死亡阶段交互），语义可镜像的另按 `docs/differential_sabberstone.md` 做 SabberStone 牌组级一致性对照；机制保留，供未来新增卡使用。
