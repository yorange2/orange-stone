...
> 已归档 2026-08-06 —— 全部里程碑 M1~M3 完成（PR #92–#94），实现细节见 git 历史。本路线图清偿了已登记的保真债 **F-A10**（`docs/fidelity-debt.md`）：空牌库抽牌现在按官方规则造成疲劳伤害（1、2、3、……），抽干牌库的对局以真实胜者结束，`max_steps` / `max_turns` 降级为兜底。英文镜像：`fatigue-roadmap.md`（在 `docs/finished/`）。
...
### M2 — 覆盖与交互 ✅
- [x] 逐条审计所有抽牌路径确认都经咽喉点（回合抽牌、`DrawCard`、`ChanceDraw`、`DrawAndDamageByCost`、亡语、生命分流、远视、战斗怒火、照明弹）；任何未覆盖的直接 `zones().iter(Zone::Deck)` 移动需给出决策（多半是 D4 式"不疲劳"或重构）
- [x] **F-A10 场景正式钉住**：双方空牌库 → 对局远在 `max_steps` 之前以真实胜者结束（用疲劳致死测试替代旧的"限步平局"预期）
- [x] 寒冰屏障：疲劳致死被阻止，对局继续
- [x] 确定性：疲劳不使用 RNG —— 同 seed 重放逐位一致（既有确定性测试必须原样通过）
- [x] SabberStone 差分用例覆盖空牌库对局（`tests/differential.rs`，若对照矩阵覆盖到疲劳分支）
- [x] `cargo bench` 抽查：疲劳是冷路径（每局至多约 30 次），热路径无回退
