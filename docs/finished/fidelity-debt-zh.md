...
### 32. MEND W4 —— 大灾变职业套装 W4 波，圣骑士（7 张卡：MEND_800~805 + MEND_900）🔓 登记

MEND W4 波（`src/cards/exp_cata_w8.rs`）的简化登记：第四个 MEND_ 波
（2025–2026 扩展总路线图 M5 的跟进）——大灾变圣骑士职业套装，7 张卡
（MEND_800 蛮勇战帅、MEND_801 韧性救主、MEND_802 复原术、MEND_803 鼓舞之刃、
MEND_804 救赎者阿拉托尔、MEND_805 慈善、MEND_900 协同作战），注册于
`sets.rs`（7 条手写条目）。与 §14–§31 相同，这些卡不在 RL 池内，所以各行
仅为信息性登记：让代码里的简化可追溯到账本。

该波的头条机制——**白银之手新兵整局永久增益**——是完整原语：蛮勇战帅的
亡语（+1 攻击力）、韧性救主的触发（+1 生命值）与鼓舞之刃的战吼（+1/+1）
"使你本局游戏中的白银之手新兵"获得永久属性加成。两个 Player 旗标
（`silver_hand_attack_bonus` / `silver_hand_health_bonus`，`src/core/player.rs`）
累积加成，并在**每一个 CORE_GVG_061t 生成点**应用——召唤结算
（`resolve_summon_doubled`）与入手路径（`add_card_to_hand`）——因此场上、
手牌与从手牌打出的新兵在本局剩余时间内全部受其影响。官方文本是无区域
限制的光环式增益；这两个钩子是引擎仅有的衍生物生成点（引擎没有圣骑士
英雄技能），所以旗标模型是完备的。

该波的 §32 钉（已登记约定）：**韧性救主的指向钉**——触发挂在随从身上，
监听 `TriggerEvent::DivineShieldLost`（福特雷根事件，由伤害管线以失去圣盾
的随从为事件主体触发）；结算臂检查事件主体，因此只有该随从自己的圣盾
破碎才获得加成（全局作用域的触发若没有这个钉，会把每次友方圣盾破碎都
重复计数）；**阿拉托尔只翻倍当前场面**——官方裁定把"所有友方白银之手
新兵"理解为当时的当前场面：战吼只翻倍结算那一刻在场的新兵（附魔等于其
当前属性——已受整局增益的新兵翻倍完整有效属性，2/1 → 4/2）并授予嘲讽；
之后召唤的新兵保留整局增益但不再翻倍、也没有嘲讽；**复原术的固定 {0}**——
召唤的新兵是 1/1（圣骑士职业套装没有为其提供升级机制）；圣盾落在效果
拿回的那只召唤物上，因此被卡德加翻倍的孪生召唤物没有圣盾（D2）；
**慈善的本回合死亡快照**——复制对象取自 `Player::died_this_turn`（死亡
步骤写入、拥有者回合结束时清空），是纯友方随从快照；+3/+3 直接烘焙进
复制体的基础属性（格里姆街手牌增益约定），满手牌时加牌被吞（F-A11）；
**协同作战的均分**——"召唤并获得四张 1/1 白银之手新兵"按 2+2 均分结算：
召唤两只、入手两张（D2）。

该波的 D2 决策（2026-08-12 登记，对照官方卡文本与 35.4.2 补丁数据核实）：
1. **协同作战均分（2+2）**——召唤两只新兵、入手两张新兵；官方文本未规定
   分配方式。
2. **阿拉托尔只翻倍当前场面**——之后召唤的新兵不受影响（官方裁定）。
3. **复原术的基础是固定 1/1**——圣骑士职业套装没有为其 {0} 提供升级机制。
4. **被卡德加翻倍的孪生召唤物没有圣盾**——圣盾只授予效果结算的那只召唤物
   （翻倍的孪生是引擎后续追加召唤，超出该效果的作用范围）。
5. **整局增益同样作用于入手路径**——官方"本局游戏"增益无区域限制；手牌
   复制与从手牌打出的新兵同样受旗标影响，因此钩子同样放在
   `add_card_to_hand` 中。

| ID | 卡牌 | 简化 | 真实形态 |
| --- | --- | --- | --- |
| MEND_800 | 蛮勇战帅 | 完整：突袭（关键字名单）+ 亡语为本局剩余时间设置 +1 攻击力旗标 | — |
| MEND_801 | 韧性救主 | 完整：带圣盾的 3/1 德莱尼；DivineShieldLost 触发（指向钉）设置 +1 生命值旗标 | — |
| MEND_802 | 复原术 | 完整：两只带圣盾的 1/1 新兵（固定的 {0} 基础） | 职业套装的 {0} 升级机制（如有） |
| MEND_803 | 鼓舞之刃 | 完整：3/2 武器 + 战吼设置 +1/+1 旗标 | — |
| MEND_804 | 救赎者阿拉托尔 | 完整：战吼只翻倍当前场面的新兵（附魔 = 当前属性）并授予嘲讽 | — |
| MEND_805 | 慈善 | 完整：本回合死亡的友方随从的复制体，+3/+3 烘焙进复制体基础属性；满手牌时加牌被吞（F-A11） | — |
| MEND_900 | 协同作战 | 完整：2+2 均分——召唤两只、入手两张（D2） | 官方的分配方式定义（如有） |

F5 覆盖：`mend_w4_*`（`tests/differential.rs` 7 个场景）——
`mend_w4_brash_battlemaster_deathrattle_gives_recruits_attack`（蛮勇战帅在
交换中死亡，之后的复原术召唤出 2/1 且带圣盾的新兵）、
`mend_w4_resilient_savior_health_pinned_to_own_shield`（两只韧性救主、两次
圣盾破碎——指向钉使新兵为 1/3；若缺少钉则会是 1/5）、
`mend_w4_convalescence_summons_divine_shield_recruits`（两只普通 1/1 带圣盾；
2/2 的命中被圣盾吸收、新兵存活）、`mend_w4_emboldening_blade_buffs_recruits_and_equips_weapon`
（英雄装备 3/2 武器、两个旗标均设置、之后的新兵为 2/2）、
`mend_w4_arator_doubles_board_recruits_with_taunt`（2/1 → 4/2 且带嘲讽；之后
召唤的新兵保持 2/1 无嘲讽）、`mend_w4_charity_copies_died_this_turn_buffed`
（交换死亡的猛禽与鱼人被复制为 6/5 与 5/4，存活者不被复制）、
`mend_w4_teamwork_summons_two_and_gets_two`（召唤两只 2/1 新兵、入手两张
2/1 新兵（1 费），打出一张手牌复制后上场）。全部 `cargo test` 全绿（所有
套件，含全部既有场景），`cargo fmt` 干净，`cargo clippy --all-targets`
零警告。
