...
- [x] **F4 ongoing** — Per-card fidelity backlog: **67 simplified-card markers** in `src/cards/` (see the canonical ledger [docs/fidelity-debt.md](../fidelity-debt.md), zh: `fidelity-debt-zh.md`). A card leaves the ledger only when implemented + F5-verified. Structural findings from the 2026-08-06 pass (stale comments, Worgen one-liner, 3 ID collisions, 10 cards absent from `ALL_CARDS`, 7 duplicated entries, debt-extractor misattribution) are **resolved (PR #77)**. **ALL CLEARED (2026-08-06, PR #79–#86)**: the 8 dependency-ordered waves (W0 wiring … W7 wrap-up) of [docs/finished/fidelity-debt-roadmap.md](fidelity-debt-roadmap.md) all landed, the ledger is empty, and the RL pool reaches the full classic constructed size (391); every card left the ledger with an F5 differential scenario. Post-hoc correction: Pilfer's OtherClass pool was narrowed from "any non-Rogue card" to the other eight classes' class cards (neutrals are not class cards — pinned by `is_other_class_card`).
...
- [x] **F5 ongoing** — Differential coverage for the fidelity-debt ledger: **done together with the ledger (PR #79–#86)** — every card that left [docs/fidelity-debt.md](../fidelity-debt.md) landed with a `tests/differential.rs` scenario (72 `w0_*`–`w7_*` total: exact target sets / trigger timing / death-phase interplay), plus SabberStone deck-level parity where the semantics can be mirrored (`docs/differential_sabberstone.md`). The mechanism stays active for future cards, keeping any new backlog mechanically verifiable.
