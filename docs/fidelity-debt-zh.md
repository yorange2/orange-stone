...
> RL 卡池降至 361（413 − 27 债 − 硬币/衍生物）。需要重训。F-A8（8 处卡 ID 冲突）仍未解决。**
...
### 11. 2026-08-06 补登记 — 既有简化债（27 张，待清偿）
...
| SHAMAN_016 | 叉状闪电 | 过载 1，真实 2（id 冲突，见 F-A8） |
| SHAMAN_018 | 雷铸战斧 | 无过载 (1) |
| SHAMAN_011 | 毁灭之锤 | 无过载 (2) |
...
## F-A8 — 8 处卡 ID 冲突 + 过载接线错位（未解决，2026-08-06 发现）
...
修复计划（代码轮，不静默做——涉及引擎 + F5 场景）：每对重排一张卡的 ID 为
唯一值（优先用真实炉石 ID，F-A3 先例），过载匹配表按实际卡 ID 重接并修正
（叉状闪电 2、补雷铸战斧 1 与毁灭之锤 2、删掉风怒 / 风语者 / 先祖之魂的
幻影条目），补 F5 差分场景。已核实：RL 侧不引用这 8 个 ID（hearthstone_os /
hearthstone 只用 CLASSIC_*/NEUTRAL_* 系列）——无需改 RL 卡组配置。
...
7. 减费（海巨人、伺机待发）；连击基础分支（冷血）；过载（雷铸战斧、毁灭之锤、
   叉状闪电的量——见 F-A8）
