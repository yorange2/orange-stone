...
> F-A8（PR #88）解决 3 张，剩 **24 张**。每张卡只有在真实炉石效果实现**并且**
...
### W8 — 触发接线（5 张）：阿曼尼狂战士、暴怒的狼人、格罗玛什、战歌指挥官、北郡牧师

- 经 `ThisMinionDamaged` 关键词表接线激怒（W0 先例）：阿曼尼狂战士
  （2/3 → 激怒 5/3）、暴怒的狼人（1/3 风怒 → 激怒 3/3 风怒）、格罗玛什
  （4/9 → 激怒 7/9）
- 战歌指挥官：其他友方随从全量 `GrantCharge` 光环（苔原犀牛先例，排除自身）
- 北郡牧师：友方 `CharacterHealed` 时抽一张

**验收**：5 个 `w8_*` 差分场景（激怒结算顺序、冲锋光环授予、治疗抽牌时机相对
治疗事件）；`cargo test` 全绿；RL 池 367 → 372；§11 删 5 行。

### W9 — 武器与种族（4 张）：真银圣剑、血吼、鹰角弓、狂野怒火

- 真银圣剑：英雄持剑攻击时治疗 2（`Attacked` 先例）
- 血吼：英雄攻击随从时武器失去 1 点攻击（附魔而非耐久）
- 鹰角弓：每有友方奥秘打出 +1 耐久（`SecretPlayed` 先例）
- 狂野怒火：既有 `GrantAttackAndImmune` 效果的目标改为 `FriendlyRace(Beast)`

**验收**：4 个 `w9_*` 场景（攻击时治疗时机、逐次攻击减攻、每奥秘加耐久、
野兽限定目标）；`cargo test` 全绿；RL 池 372 → 376。

### W10 — 抉择与发现（5 张）：愤怒、利爪德鲁伊、知识古树、战争古树、追踪术

- 愤怒 / 利爪德鲁伊 / 知识古树 / 战争古树：把两个分支接进既有抉择管道
  （先审计 def 为何落到固定分支——多半是缺第二分支接线，不是管道缺口）
- 追踪术：从**玩家牌库顶 3 张**中发现——选 1 入手，弃掉其余。发现管道已有但
  池来源是卡池；决定（D1）牌库顶 3 张是新池来源还是并行选择

**验收**：5 个 `w10_*` 场景（各分支选择、分支的费用/目标、追踪术牌库顶 3 张
语义含弃牌）；`cargo test` 全绿；RL 池 376 → 381。

### W11 — 战吼与减费（6 张）：奥妮克希亚、阿古斯保护者、死亡之翼、海巨人、伺机待发、冷血

- 奥妮克希亚：召唤五只 1/1 幼龙（`SummonMinion` 循环）
- 阿古斯保护者：相邻随从 +1/+1 **并且**获得圣盾（目标结构 + 圣盾授予；W5 邻位
  先例）
- 死亡之翼：**弃掉整个手牌**（新效果——`DiscardRandomCard` 已有，全弃没有）+
  摧毁所有其他随从
- 海巨人：场上每有一个随从费用 -1（按场随从数减费光环——新；W3
  `FirstMinionDiscount` 先例）
- 伺机待发：本回合下一个法术费用 -3（每玩家标志，米尔豪斯 `spells_cost_zero`
  W4 先例）
- 冷血：基础 +2 分支与既有连击分支一起接线

**验收**：6 个 `w11_*` 场景（幼龙数量与满场、阿古斯邻位含圣盾授予顺序、死亡之翼
全弃 + 清场、海巨人费用随场面、伺机待发窗口、冷血基础分支）；`cargo test` 全绿；
RL 池 381 → 387。
