...
- [ ] **F4 ongoing** — Per-card fidelity backlog: **67 simplified-card markers** in `src/cards/` (see the canonical ledger [docs/finished/fidelity-debt.md](finished/fidelity-debt.md), zh: `fidelity-debt-zh.md`). A card leaves the ledger only when implemented + F5-verified. Structural findings from the 2026-08-06 pass (stale comments, Worgen one-liner, 3 ID collisions, 10 cards absent from `ALL_CARDS`, 7 duplicated entries, debt-extractor misattribution) are **resolved (PR #77)**; what remains is the per-mechanism work: Enrage, tribe field, missing trigger classes, conditional predicates, and the effect primitives listed in the ledger's mechanism inventory. **Execution plan: [docs/finished/fidelity-debt-roadmap.md](finished/fidelity-debt-roadmap.md)** (zh: `fidelity-debt-roadmap-zh.md`) — 8 dependency-ordered waves (W0 wiring … W7 wrap-up). The RL training pool (`hearthstone_os/decks.py`) excludes this set, so pool membership changes flow through the ledger.
...
- [ ] **F5 ongoing** — Differential coverage for the fidelity-debt ledger: every card leaving [docs/finished/fidelity-debt.md](finished/fidelity-debt.md) must land with a `tests/differential.rs` scenario (exact target sets / trigger timing / death-phase interplay) plus a SabberStone deck-level parity check where the semantics can be mirrored (`docs/differential_sabberstone.md`). This keeps the F4 backlog mechanically verifiable.
