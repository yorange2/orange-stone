...
Status: milestones A–G are complete (PRs #35–#62); Milestone F is complete (PRs #61, #63, #75: F1 overload lock, F2 stealth, F4 fidelity audit, F5 differential validation incl. the external SabberStone run). Remaining: the ongoing per-effect audit carried by F5 — its backlog is the fidelity-debt ledger ([docs/fidelity-debt.md](fidelity-debt.md): 68 simplified-card markers, 2026-08-06 pass).
...
- [ ] **F5 ongoing** — Differential coverage for the fidelity-debt ledger: every card leaving [docs/fidelity-debt.md](fidelity-debt.md) must land with a `tests/differential.rs` scenario (exact target sets / trigger timing / death-phase interplay) plus a SabberStone deck-level parity check where the semantics can be mirrored (`docs/differential_sabberstone.md`). This keeps the F4 backlog mechanically verifiable.
