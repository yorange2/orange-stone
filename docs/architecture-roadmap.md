...
Status: milestones A–G are complete (PRs #35–#62); Milestone F is complete (PRs #61, #63, #75: F1 overload lock, F2 stealth, F4 fidelity audit, F5 differential validation incl. the external SabberStone run). The F4 per-effect audit backlog (67 simplified-card markers) is fully cleared (PRs #79–#86, W0 wiring … W7 wrap-up); the ledger and roadmap are archived under docs/finished/. The F4/F5 ongoing-audit mechanism remains — future simplified cards register per the archived ledger's maintenance conventions.
...
- ✅ **RESOLVED (F1, F2, PR #61)** — Overload was only a trigger marker; Stealth was permanent and its target-side claim unenforced. Now `Overload(pub i32)` carries per-card amounts (Lightning Bolt 1, Feral Spirit 2, Forked Lightning 2, Lightning Storm 2, Totem Golem 1, Lava Burst 2, Earth Elemental 3); playing an overload card accumulates `Player::overload_locked`, applied (and cleared) at the next ManaRefill step — the G5 site. Stealth's target side landed with G9 (stealthed characters cannot be explicitly targeted — the effect whiffs); the attack side lands here: attacking removes the attacker's Stealth.
...
- ✅ **RESOLVED (E4, PR #62)** — The card database was ~400 handwritten `const CardDef`s and the build.rs pipeline shipped only 4 sample cards. Now the full official database is in the repo (`cards/cards.json`: the HearthstoneJSON classic-era constructed sets — EXPERT1 + CORE + LEGACY, deduplicated by ID, 1190 cards): build.rs generates the complete static card constants plus `find_by_id`/`find_by_name` lookups (weapon durability lives in the official health field — mapped; the name index prefers real cards over enchantment tokens). The handwritten pool (custom IDs) is verified against the official database field-by-field **by name**, with a ~90-entry ledger of documented official rebalances (`known_rebalanced`). Overlaying handwritten effects onto official IDs (name→ID mapping) is follow-up work.
...
- [x] **E2** — Retire the last queue-mutation primitive `redirect_damages` (B2 leftover): route Spellbender through the damage pipeline (e.g. damage-source interception at `DamageDealt`) instead of rewriting pending spell-damage events. *(⏩ subsumed by G8: step-boundary interception replaces queue mutation — done via G8, PR #59)*
- [x] **E3** — Choose One / Combo decision surface (P2): expose the choice in `Action` so the agent decides instead of engine-random / auto-detected combo. *(⏩ superseded by G6: the Choice system covers it — done via G6, PR #57)*
- [x] **E4** — Complete D2: vendor the full official CardDefs JSON and regenerate the static card consts (hand-written effect fields stay on top of the generated statics). *(PR #62: `cards/cards.json` (1190 cards) + full generation + name-based verification with a rebalance ledger; the effect-overlay name→ID mapping is follow-up)*
...
- [x] **G4** — Enchantment layer: stats become base + enchantment deltas (+ damage) instead of direct component writes; buff/debuff/cost effects attach enchantments, auras stay query-time. Enables correct Silence (strips enchantments only), transform, copy (Faceless), and "until end of turn" expiry. *(PR #55: `Enchantment` + `Damage` components; set-to-value modifiers deferred to G5)*
...
- [x] **F1** — Overload mana lock: implement real Hearthstone semantics (per-card overload amounts lock the owner's mana on the next turn) instead of the trigger-only marker. *(unblocked by G1/G5: the lock applies at the mana-refill step)* *(PR #61: `Overload(i32)` + `overload_locked`, deducted at ManaRefill)*
- [x] **F2** — Stealth full fidelity: remove Stealth when the character attacks, and exclude stealthed characters from single-target effects (currently only attacks are blocked). *(targeting half lands with G9)* *(PR #61: attacking removes Stealth; targeting half via G9)*
- [x] **F3** — Choose One player choice: expose the choice in `Action` (engine-random selection is not faithful). *(⏩ superseded by G6: the Choice system covers it — done via G6, PR #57)*
- [x] **F4** — Fidelity audit: systematic per-`CardEffect` pass against real Hearthstone semantics; fix known divergences (e.g. single-target destroy currently destroys *all* matching enemies; damaged-enemy destroy likewise; verify target sets, damage sequencing, aura stacking). *(blocked by G2–G5: per-effect fixes are only meaningful once the sequencing/enchantment primitives exist)* *(PR #63: single-target destroy picks one random + damaged predicate fix + aura stacking tests; the ongoing audit is carried by F5)*
- [x] **F4 ongoing** — Per-card fidelity backlog: **67 simplified-card markers** in `src/cards/` (see the canonical ledger [docs/finished/fidelity-debt.md](finished/fidelity-debt.md), zh: `fidelity-debt-zh.md`). A card leaves the ledger only when implemented + F5-verified. Structural findings from the 2026-08-06 pass (stale comments, Worgen one-liner, 3 ID collisions, 10 cards absent from `ALL_CARDS`, 7 duplicated entries, debt-extractor misattribution) are **resolved (PR #77)**. **ALL CLEARED (2026-08-06, PR #79–#86)**: the 8 dependency-ordered waves (W0 wiring … W7 wrap-up) of [docs/finished/fidelity-debt-roadmap.md](finished/fidelity-debt-roadmap.md) all landed, the ledger is empty, and the RL pool reaches the full classic constructed size (391); every card left the ledger with an F5 differential scenario. Post-hoc correction: Pilfer's OtherClass pool was narrowed from "any non-Rogue card" to the other eight classes' class cards (neutrals are not class cards — pinned by `is_other_class_card`).
- [x] **F5** — Differential validation: conformance tests comparing game outcomes / event sequences against SabberStone (and/or RosettaStone) as reference implementations, so fidelity regressions are caught mechanically. *(depends on G1–G9: differential runs compare step-level sequencing)* *(PR #63: `tests/differential.rs` reference scenarios + transcript protocol doc; PR #75: external SabberStone run — dotnet-driven mirror of the attack-trade scenarios, `docs/differential_sabberstone.md`)*
- [x] **F5 ongoing** — Differential coverage for the fidelity-debt ledger: **done together with the ledger (PR #79–#86)** — every card that left [docs/finished/fidelity-debt.md](finished/fidelity-debt.md) landed with a `tests/differential.rs` scenario (72 `w0_*`–`w7_*` total: exact target sets / trigger timing / death-phase interplay), plus SabberStone deck-level parity where the semantics can be mirrored (`docs/differential_sabberstone.md`). The mechanism stays active for future cards, keeping any new backlog mechanically verifiable.
