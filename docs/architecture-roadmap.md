...
Status: milestones A–G are complete (PRs #35–#62); Milestone F is complete (PRs #61, #63: F1 overload lock, F2 stealth, F4 fidelity audit, F5 differential validation). Remaining: the external SabberStone comparison run (needs a .NET environment) and the ongoing per-effect audit carried by F5.
